...
export default function TeacherWeeklyReportPage() {
  const t = useTranslations("teacherWeeklyReportPage")
  const tDetail = useTranslations("teacherDetailPage")
        ...
        <div className="py-12 text-center space-y-3" role="alert">
          ...
          <Link href="/school/teachers">
            <Button variant="secondary">
              <ArrowLeft className="h-4 w-4" /> {t("backToTeachers")}
            </Button>
          </Link>
        </div>
      </Card>
    ...
    <div className="space-y-6 print:space-y-4">
      <div className="flex items-center justify-between print:hidden">
        <Link href={`/school/teachers/${data.teacher.id}?weekStart=${weekStart}`}>
          <Button variant="secondary">
            <ArrowLeft className="h-4 w-4" /> {t("backToTeacherProfile")}
          </Button>
        </Link>
        ...
        <div className="space-y-6 print:space-y-4">
          <div className="border-b pb-4 print:pb-3">
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="text-left">
                <Badge variant="info">
                  <Calendar className="h-3 w-3" /> {t("weekRange", { start: data.payroll.weekStart, end: data.payroll.weekEnd })}
                </Badge>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
            <div className="rounded-xl border border-gray-200 px-4 py-3">
              ...
              <p className="font-semibold">{t("daysCount", { count: data.payroll.attendanceSummary.presentDays })}</p>
            </div>
            <div className="rounded-xl border border-gray-200 px-4 py-3">
              <p className="text-sm text-gray-500">{t("absence")}</p>
              <p className="font-semibold">{t("daysCount", { count: data.payroll.attendanceSummary.absentDays })}</p>
            </div>
            <div className="rounded-xl border border-gray-200 px-4 py-3">
              <p className="text-sm text-gray-500">{t("recordedHours")}</p>
              <p className="font-semibold">{t("hoursCount", { count: data.payroll.totalHours })}</p>
            </div>
            <div className="rounded-xl border border-gray-200 px-4 py-3">
              <p className="text-sm text-gray-500">{t("estimatedPay")}</p>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
            <div className="rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm">
              <p className="font-medium text-gray-900">{t("howToReadTitle")}</p>
              <p className="mt-2 text-gray-600">{t("howToReadText")}</p>
            </div>
            <div className="rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm">
              <p className="font-medium text-gray-900">{t("incompletePayTitle")}</p>
              <p className="mt-2 text-gray-600">{t("incompletePayText")}</p>
            </div>
            <div className="rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm">
              <p className="font-medium text-blue-900">{t("reviewTitle")}</p>
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <div>
              <h2 className="mb-3 flex items-center gap-2 text-lg font-semibold"><ShieldCheck className="h-5 w-5" /> {t("attendanceLog")}</h2>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b text-gray-500">
                      ...
                      <th className="px-3 py-2 text-right">{t("markedBy")}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.payroll.attendanceRecords.length === 0 ? (
                      <tr>
                        <td colSpan={3} className="px-3 py-6 text-center text-gray-400">{t("noAttendanceRecords")}</td>
                      </tr>
                    ) : data.payroll.attendanceRecords.map((record) => (
                      <tr key={record.id} className="border-b">
                        <td className="px-3 py-2">{record.date}</td>
                        <td className="px-3 py-2 text-center">{record.status === "PRESENT" ? tDetail("present") : record.status === "ABSENT" ? tDetail("absent") : record.status === "LATE" ? tDetail("late") : record.status === "EXCUSED" ? tDetail("excused") : record.status}</td>
            ...
            <div>
              <h2 className="mb-3 flex items-center gap-2 text-lg font-semibold"><Clock className="h-5 w-5" /> {t("timeline")}</h2>
              <div className="space-y-2">
                {data.payroll.activityTimeline.length === 0 ? (
                  <p className="py-6 text-center text-sm text-gray-400">{t("noTimeline")}</p>
          ...
          <div>
            <h2 className="mb-3 flex items-center gap-2 text-lg font-semibold"><Wallet className="h-5 w-5" /> {t("assignmentsSummary")}</h2>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b text-gray-500">
                    <th className="px-3 py-2 text-right">{t("subject")}</th>
                    <th className="px-3 py-2 text-right">{t("classroom")}</th>
                    <th className="px-3 py-2 text-center">{t("recorded")}</th>
                    <th className="px-3 py-2 text-center">{t("hourlyRate")}</th>
                    <th className="px-3 py-2 text-center">{t("estimatedPayColumn")}</th>
                  </tr>
                </thead>
                <tbody>
